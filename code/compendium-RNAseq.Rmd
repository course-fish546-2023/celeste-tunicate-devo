...
output:
  html_document:
    ...
    toc_float: true
    toc_depth: 3
    css: "celeste-tunicate-devo/style.css"
---
...
```{r}
# Create a matrix to store the samples grouped by conditions
sample_groups <- split(colnames(log_counts_top), deseq2.colData$condition)

# Sort the sample groups
sample_groups <- sample_groups[order(names(sample_groups))]

# Define the desired condition labels
condition_labels <- c("b5-b6", "TO")

# Extract column labels based on condition labels
column_labels <- unlist(lapply(sample_groups, function(x) condition_labels[which(x %in% colnames(log_counts_top))]))

# Generate heatmap with sample groups
pheatmap(log_counts_top, scale = "row", cluster_rows = TRUE, cluster_cols = TRUE,
         labels_col = column_labels, labels_row = FALSE,
         color = colorRampPalette(c("blue", "white", "red"))(100),
         breaks = seq(-3, 3, length.out = 101), show_rownames = FALSE)
```
...
counts_pro <- merge(countsdf, blast_go, by = "access_num", all = FALSE) 
