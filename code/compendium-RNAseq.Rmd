...
knitr::opts_chunk$set(
  ...
  fig.width = 13,       # Set plot width in inches
...
Merge blast data frame and count data frame by an inner join so that we can just label genes that exist in the countsdf data frame. Note that there will be duplicate protein entries in this new merged data frame that got annotated the same in BLASTing. This is normal and so we are just going to get rid duplicates so that we are acutlaly able to visualize whats going on.
```{r, eval=TRUE, cache=TRUE}
